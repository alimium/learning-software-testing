...
def get_user_repository(db: Session = get_db()) -> SQLAlchemyUserRepository:
    """Get user repository."""
    return SQLAlchemyUserRepository(db)


def get_venue_repository(db: Session = get_db()) -> SQLAlchemyVenueRepository:
    """Get venue repository."""
    return SQLAlchemyVenueRepository(db)


def get_event_repository(db: Session = get_db()) -> SQLAlchemyEventRepository:
    """Get event repository."""
    return SQLAlchemyEventRepository(db)


def get_seat_repository(db: Session = get_db()) -> SQLAlchemySeatRepository:
    """Get seat repository."""
    return SQLAlchemySeatRepository(db)


def get_order_repository(db: Session = get_db()) -> SQLAlchemyOrderRepository:
    """Get order repository."""
    return SQLAlchemyOrderRepository(db)


def get_auth_service(
    user_repo: SQLAlchemyUserRepository = get_user_repository(),
) -> AuthService:
    """Get auth service."""
...
def get_event_service(
    event_repo: SQLAlchemyEventRepository = get_event_repository(),
) -> EventService:
    """Get event service."""
...
def get_order_service(
    order_repo: SQLAlchemyOrderRepository = get_order_repository(),
    event_repo: SQLAlchemyEventRepository = get_event_repository(),
    seat_repo: SQLAlchemySeatRepository = get_seat_repository(),
) -> OrderService:
